...
export class ConfigurationRepository {
  ...
  async _handlerConfigurationRequest(configurationRequest) {
    ...
    try {
      ...
      let value
      switch (configurationRequest.type) {
        case ConfigurationRequestTypes.local:
          value = configurationRequest.key
          break
        case ConfigurationRequestTypes.ssm:
          value = await this._ssmRepository.getParameter(configurationRequest.key)
          break
        case ConfigurationRequestTypes.secret:
          value = await this._secretsRepository.getSecret(configurationRequest.key)
          break
        default:
          break
      }
