...
test('getConfiguration should get all requested local parameters', async (t) => {
  const configurationRepository = new ConfigurationRepository(t.context)
  ...
  const config = await configurationRepository.getConfiguration(configurationRequests)
  t.like(config, {
    [configurationRequests[0].propertyName]: configurationRequests[0].key,
    [configurationRequests[1].propertyName]: configurationRequests[1].key
  })
})
